...
def _fetch_from_twelvedata(kode: str) -> dict | None:
    """
    Fetch dari Twelve Data API — 800 req/hari gratis, cloud-friendly.
    IDX Indonesia: format BBRI, ANTM, dst (tanpa .JK)
    ...
    symbol = f"{kode}:IDX"   # format Twelve Data untuk IDX Indonesia
    ...
    r.raise_for_status()
    q = r.json()
    logger.info(f"TwelveData quote {kode}: status={q.get('status')} code={q.get('code')}")

    if q.get("status") == "error" or q.get("code") == 400:
        logger.warning(f"TwelveData: quote error for {kode}: {q.get('message')}")
        return None

    try:
        current = float(q.get("close") or q.get("price") or 0)
    ...
    if current == 0:
        logger.warning(f"TwelveData: price=0 for {kode}")
        return None

    # ── Data historis OHLCV (180 hari)
    r2 = requests.get(
        f"{base}/time_series",
        params={
            "symbol":     symbol,
    ...
    if ts.get("status") == "error" or not ts.get("values"):
        logger.warning(f"TwelveData: no timeseries for {kode}: {ts.get('message')}")
        return None

    values = ts["values"]  # list of dict, newest first
    values = list(reversed(values))  # urutkan oldest → newest
    ...
    try:
        ...
        volumes = pd.Series([float(v.get("volume", 0) or 0) for v in values], dtype=float)
        dates   = pd.to_datetime([v["datetime"] for v in values])
    except (KeyError, TypeError, ValueError) as e:
        logger.warning(f"TwelveData: timeseries parse error {kode}: {e}")
        return None
    ...
    if len(hist_df) < 20:
        logger.warning(f"TwelveData: not enough data ({len(hist_df)} rows) for {kode}")
        return None
