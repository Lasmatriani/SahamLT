...
def get_price_data(kode: str) -> dict | None:
    ...
    1. Twelve Data (primary — 800 req/hari gratis, cloud-friendly)
    2. yfinance (fallback)
    """
    kode = kode.upper().strip()

    for source, func in [
        ("TwelveData", lambda: _fetch_from_twelvedata(kode)),
...
def _fetch_from_twelvedata(kode: str) -> dict | None:
    """
    Fetch dari Twelve Data API.
    Free tier: 800 req/hari, cloud-friendly, IDX Indonesia support.
    Docs: https://twelvedata.com/docs
    Symbol IDX: symbol=BBRI, exchange=IDX  (BUKAN BBRI:IDX)
    """
    import requests

    if not TWELVEDATA_API_KEY:
        logger.warning("TWELVEDATA_API_KEY tidak diset")
        return None
    ...
    # ── Quote (harga terkini + previous close + change%)
    r = requests.get(f"{base}/quote", params=params_base, timeout=10)
    r.raise_for_status()
    q = r.json()
    logger.info(f"TwelveData quote {kode}: status={q.get('status')} symbol={q.get('symbol')}")

    if q.get("status") == "error":
        logger.warning(f"TwelveData: quote error {kode}: {q.get('message')}")
        return None

    try:
        current = float(q.get("close") or 0)
        prev    = float(q.get("previous_close") or current)
        name    = q.get("name") or kode
        chg_pct = float(q.get("percent_change") or 0)
        if chg_pct == 0 and prev > 0:
            chg_pct = ((current - prev) / prev * 100)
    except (TypeError, ValueError) as e:
        logger.warning(f"TwelveData: quote parse error {kode}: {e}")
        return None

    if current == 0:
        logger.warning(f"TwelveData: price=0 for {kode}, response={q}")
        return None

    # ── Time series (OHLCV 180 hari, newest-first → kita reverse)
    ts_params = {**params_base, "interval": "1day", "outputsize": 180, "order": "asc"}
    r2 = requests.get(f"{base}/time_series", params=ts_params, timeout=15)
    r2.raise_for_status()
    ts = r2.json()
    logger.info(f"TwelveData timeseries {kode}: status={ts.get('status')} count={len(ts.get('values', []))}")

    if ts.get("status") == "error" or not ts.get("values"):
        logger.warning(f"TwelveData: no timeseries {kode}: {ts.get('message')}")
        return None

    values = ts["values"]  # sudah asc (oldest first) karena order=asc

    try:
        opens   = pd.Series([float(v["open"])              for v in values], dtype=float)
        highs   = pd.Series([float(v["high"])              for v in values], dtype=float)
        lows    = pd.Series([float(v["low"])               for v in values], dtype=float)
        closes  = pd.Series([float(v["close"])             for v in values], dtype=float)
        volumes = pd.Series([float(v.get("volume") or 0)  for v in values], dtype=float)
        dates   = pd.to_datetime([v["datetime"]            for v in values])
    except (KeyError, TypeError, ValueError) as e:
        logger.warning(f"TwelveData: parse error {kode}: {e}. Sample: {values[0] if values else 'empty'}")
        return None
    ...
    if len(hist_df) < 20:
        logger.warning(f"TwelveData: data kurang ({len(hist_df)} rows) untuk {kode}")
        return None

    logger.info(f"TwelveData: {kode} price={current} rows={len(hist_df)}")
